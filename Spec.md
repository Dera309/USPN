...
---
## Input:
[I want you to build a cargo site with shipment tracking, booking and management shipment, login page, user account, admin dashboard that manages cargo and user data, it should be able to allow the admin to change the cargo location and status and also give tracking number. no payment processing. it should have a quote calculator. use the designs in the stitch folder for the Ui design. use google maps static maps api for the map. the website name is USPN.    ASK THE USER]
## Output:
